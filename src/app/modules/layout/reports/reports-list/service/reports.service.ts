...
import { GET_ALL_ROSTER, COMPANY_SHIFT, ADMIN_DETAIL, ADD_NOTIFICATION, SCOPE_API, SNAPSHOT_API, SNAPSHOT_VACANTS, SNAPSHOT_OCCUPIED, GET_LOCATIONS, OCCUPANCY_VARIATION, ANPR_REPORTS } from '../../../../../constant/urls';
...
export class ReportsService {
  ...
  getSnapshotData(data: any) {
    return this._http.post<Snapshot.List>(SNAPSHOT_API, {"timestamp":data.timestamp, "user":data.user, "password":data.password, "buildingUrl": data.buildingUrl, "camera_ids":data.cameras,"with_image":data.withImage,"block_id":data.zone,"klass":"","with_crops":true});
  }
  ...
  getAnprReportsData(data: any) {
    return this._http.post<any>(ANPR_REPORTS, {"fromDate": data.fromDate, "toDate": data.toDate, 'compLocationName':data.compLocationName});
  }
}
